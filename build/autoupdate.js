...
async function CheckUpdates(){
    ...
    if(SaveData().ui.version < latestData.ui.version){
        ...
        let modulePath = isDev ? path.resolve(__dirname, "../", moduleName) : path.resolve(process.resourcesPath, "../", moduleName)
        //Alert if the exploit is patched. Also reject downloading the update.
        if(latestData.dll.patched){
        ...
        //Downloads the update if exploit-main not exist or out of date.
        else if(!fs.existsSync(modulePath) || (SaveData().downloadedModuleVersion||0) < latestData.dll.version){
...
async function DownloadModule(downloadURL, fileName){
    ...
    let modulePath = isDev ? path.resolve(__dirname, "../", fileName) : path.resolve(process.resourcesPath, "../", fileName)
